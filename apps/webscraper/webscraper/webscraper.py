"""Module for web scraping Open Data NI"""
...
def _get_list_items(html) -> bs4.element.ResultSet:
    ...
    list_items = soup.find_all(
        "div",
        {
            "class": "item flex flex-col gap-3 lg:flex-row items-start mb-8 lg:justify-between"
        }
    )
...
def _get_url(item) -> str:
    return item.find(
        "a",
        {
            "class": "py-1 px-6 ml-2 bg-blue-c-600 rounded-lg"
        }
    ).attrs["href"]
...
def __main() -> list[dict]:
    parser = argparse.ArgumentParser(
        description="Web Scraper for datasets with tag 'GP Practice' on OpenDataNI https://www.opendatani.gov.uk/."
    )
